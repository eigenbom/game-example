...
inline std::string to_string(const ident& id){
...
public:
  ...
  value_type& operator[](key_type key){
    ...
    if (it == indices_.end()) return nullElement_;
    else return values_[it->second];
  }
...
protected:
  ...
  template <typename U>
  friend std::string to_string(const container<U>& container);
};
...
public:
  ...
  void remove(key_type key){
    ...
    bool inBuffer = std::find_if(add_.begin(), add_.end(), [key](T& value){ return value.id == key; }) != add_.end();
...
template <typename T> inline vec2<T>& operator/=(vec2<T>& a, T b)       { return a = a / b; }
template <typename T> inline bool     operator==(vec2<T> a, vec2<T> b)  { return a.x == b.x && a.y == b.y ; }
template <typename T> inline bool     operator!=(vec2<T> a, vec2<T> b)  { return a.x != b.x || a.y != b.y ; }
...
struct Array2D {
public:
  Array2D(int width, int height, T nullVal = {}): nullVal_(nullVal), width_(width), height_(height), data_(width * height, nullVal){}
...
template <typename T>
T choose(const std::vector<T>& values){
